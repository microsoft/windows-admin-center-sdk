...
export class Utils {
    ...
    public static async waitAsync(
        ...
        description: string,
        expectWaitToTimeout?: boolean): Promise<void> {
        ...
        while (true) {
            ...
            if (now - initialTime > Utils.defaultTimeout) {
                 if (!expectWaitToTimeout) {
                    fail('Wait timeout. Description: ' + description);
                }

                throw 'Wait timeout. Description:' + description;
            }
    ...
    public static async retryAsync(retryHandler: () => Promise<void>, description?: string, shouldRetry?: () => boolean,
        retryCount = 2, initialTime?: number): Promise<void> {
        if (!initialTime) {
        ...
        try {
            ...
            if (shouldRetry && shouldRetry()) {
                ...
                if (retryCount > 0) {
                    await Utils.sleepAsync(Utils.defaultWaitDuration);
                    await Utils.retryAsync(retryHandler, description, shouldRetry, retryCount - 1, initialTime);
                } else {
                    Utils.log('Action failed eventually by custom rule. Description: ' + description);
                }
            }
        } catch (e) {
            Utils.log('Action Failed. Description: ' + description + '. Retry attempts left: ' + retryCount);
            if (retryCount > 0) {
                ...
                Utils.log('Action failed eventually by throwing exception. Description: ' + description + ' | Exception: ' + e);
            }
        }
    }
    ...
    public static async clickAsync(webElement: WebElement): Promise<void> {
        try {
            if (!Settings.noColor) {
    ...
    public static async takeScreenshot() {
        ...
        fs.writeFile(Utils.screenshotsFolder + '\\record' + timestamp + '.png', data, 'base64');
    }
    ...
    private static async writeLogToFile(logContent: string, filePath: string) {
